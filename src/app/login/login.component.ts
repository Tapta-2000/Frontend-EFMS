...
export class LoginComponent implements OnInit{
  ...
  constructor(private route:Router,private guard:RouteguardService,private http:HttpClient){}
  ...
  // ngOnInit():void{
  //   const temp=window.localStorage.getItem('Registration');
  //   if(temp !=null)
  //   {
  //       this.myarray=JSON.parse(temp);
...
return newpassword;
  ...
  onlogin()
  {
    ...
    this.login.pwd=this.passwordencryption(this.login.pwd);
    const  checked=this.myarray.find(x => x.email==this.login.user && x.password==this.login.pwd);
    if(checked != undefined)
    {
      this.route.navigate(['main']);
      this.guard.myvar(true);
    }
    else{
